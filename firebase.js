...
export async function saveSteps(projectId, steps) {
  ...
  // ── AUTO-REBUILD visibleTo dari semua PIC step aktif ──
  // Ambil project existing untuk dapat owner/admin/visibleTo lama
  const projSnap = await get(ref(db, `projects/${projectId}`));
  const proj = projSnap.exists() ? projSnap.val() : {};
  const vis = {};
  // Pertahankan owner & monitor (admin)
  if (proj.visibleTo) {
    Object.entries(proj.visibleTo).forEach(([uid, role]) => {
      if (role === 'owner' || role === 'monitor') vis[uid] = role;
    });
  }
  // Pastikan org owner & actual owner tetap punya akses
  if (proj.orgOwner) vis[proj.orgOwner] = vis[proj.orgOwner] || 'owner';
  if (proj.actualOwner && !vis[proj.actualOwner]) vis[proj.actualOwner] = 'owner';
  // Tambahkan semua PIC step aktif sebagai 'execute'
  processed.forEach(s => { if (s.pic && !vis[s.pic]) vis[s.pic] = 'execute'; });
  // Pastikan minimal ada 1 admin sebagai monitor
  const hasMonitor = Object.values(vis).includes('monitor');
  if (!hasMonitor) {
    const admins = await getAdmins();
    if (admins[0]) vis[admins[0].id] = 'monitor';
  }
...
// ════════════════════════════════════════════════════════════
//  V7 — NOTIFIKASI EVENT STEP & PROJECT (terpusat)
// ════════════════════════════════════════════════════════════
// Kumpulkan semua PIC step unik dari sebuah project
function collectStepPics(project) {
  const pics = new Set();
  (project.steps || []).forEach(s => { if (s.pic) pics.add(s.pic); });
  return [...pics];
}

// Notif: PIC step menyelesaikan / membatalkan sebuah step → ke PIC Project (owner)
export async function notifyStepToggle(projectId, stepName, actorId, done) {
  const snap = await get(ref(db, `projects/${projectId}`));
  if (!snap.exists()) return;
  const p = snap.val();
  const owner = p.actualOwner || p.orgOwner;
  const targets = new Set();
  if (owner && owner !== actorId) targets.add(owner);
  for (const uid of targets) {
    await sendNotification(uid, {
      type: done ? 'step_done' : 'step_undone',
      projectId,
      title: done ? 'Step Diselesaikan' : 'Step Dibatalkan (Revisi)',
      body: done
        ? `Sebuah tahapan "${stepName}" di project "${p.name}" telah diselesaikan.`
        : `Tahapan "${stepName}" di project "${p.name}" dibatalkan penyelesaiannya untuk revisi.`,
      read: false,
    });
  }
}

// Notif: step di-uncomplete/dikoreksi oleh atasan (admin/leader) → PIC step terkait + PIC Project
export async function notifyStepCorrection(projectId, step, correctorId, correctorRole, note) {
  const snap = await get(ref(db, `projects/${projectId}`));
  if (!snap.exists()) return;
  const p = snap.val();
  const targets = new Set();
  if (step.pic && step.pic !== correctorId) targets.add(step.pic);
  const owner = p.actualOwner || p.orgOwner;
  if (owner && owner !== correctorId) targets.add(owner);
  const roleLabel = correctorRole === 'asst_manager' ? 'Admin' : correctorRole === 'leader' ? 'Leader' : 'Atasan';
  for (const uid of targets) {
    await sendNotification(uid, {
      type: 'step_correction',
      projectId,
      title: `Koreksi dari ${roleLabel}`,
      body: `Tahapan "${step.name}" di "${p.name}" dikoreksi oleh ${roleLabel}${note ? ': ' + note : '. Mohon diperbaiki.'}`,
      read: false,
    });
  }
}
...
export async function verifyCloseProject(projectId, adminId, note = '') {
  ...
  // Notif ke owner project + SEMUA PIC step
  const snap = await get(ref(db, `projects/${projectId}`));
  if (snap.exists()) {
    const p = snap.val();
    const targets = new Set(collectStepPics(p));
    if (p.actualOwner) targets.add(p.actualOwner);
    if (p.orgOwner) targets.add(p.orgOwner);
    targets.delete(adminId);
    for (const uid of targets) {
      await sendNotification(uid, {
        type:'project_closed', projectId,
        title:'Project Selesai (Diketok Palu)',
...
export async function rejectValidation(projectId, adminId = null, note = '') {
  await update(ref(db, `projects/${projectId}`), {
  ...
  if (snap.exists()) {
    ...
    if (p.actualOwner) targets.add(p.actualOwner);
    if (p.orgOwner) targets.add(p.orgOwner);
    if (adminId) targets.delete(adminId);
    for (const uid of targets) {
      await sendNotification(uid, {
        type:'validation_rejected', projectId,
        title:'Project Dikembalikan ke Open',
        body:`Admin meminta revisi pada project "${p.name}"${note?': '+note:'.'} Project kembali berstatus Open.`,
        read:false,
      });
    }
  }

...
export async function createProject(data) {
  ...
  // Ini mencegah c.val().id override c.key saat baca
  const { id: _ignore, ...dataWithoutId } = data; // strip id dari data jika ada
  const project = {
    ...dataWithoutId,
...
export async function saveSteps(projectId, steps) {
  ...
  await recalcContributions(projectId, steps);
}
...
export async function sendCollabRequest(data) {
  ...
  // Kirim notifikasi ke leader tujuan
...
export async function respondCollabRequest(reqId, accept, picId = null) {
  ...
  if (accept && picId) {
    ...
    await updateProjectVisibility(req.projectId, picId, 'execute');
    // Notif ke leader pengirim
    await sendNotification(req.fromLeaderId, {
      type:      'collab_accepted',
      reqId,
      projectId: req.projectId,
      title:     'Kolaborasi Diterima',
      body:      `Permintaan kolaborasi project "${req.projectName}" diterima`,
      read:      false,
    });
  } else if (!accept) {
    // Notif ke leader pengirim bahwa ditolak
    await sendNotification(req.fromLeaderId, {
      type:      'collab_declined',
      reqId,
      projectId: req.projectId,
      title:     'Kolaborasi Ditolak',
      body:      `Leader menolak kolaborasi project "${req.projectName}" — perlu diadakan meeting`,
      read:      false,
    });
  }
}
